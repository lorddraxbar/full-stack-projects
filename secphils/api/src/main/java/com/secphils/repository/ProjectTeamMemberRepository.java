...
public interface ProjectTeamMemberRepository extends JpaRepository<ProjectTeamMember, ProjectTeamMemberId> {
    ...
    @Query("select t from ProjectTeamMember t left join fetch t.project left join fetch t.user where t.project.id = :id")
    List<ProjectTeamMember> findWithRefsByProjectId(@org.springframework.data.repository.query.Param("id") Long id);
}
